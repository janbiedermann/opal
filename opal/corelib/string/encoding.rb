...
::Encoding.register 'UTF-8', aliases: ['CP65001'], ascii: true do
  ...
  def valid_encoding?(str)
    %x{
      let t_decoder = new TextDecoder('utf-8', { fatal: true });
      try { t_decoder.decode(new Uint8Array(str.$bytes())); }
      catch { return false; }
      return true;
    }
  end
...
::Encoding.register 'UTF-16LE', aliases: ['UTF-16'] do
  ...
  def valid_encoding?(str)
    %x{
      let t_decoder = new TextDecoder('utf-16', { fatal: true });
      try { t_decoder.decode(new Uint8Array(str.$bytes())); }
      catch { return false; }
      return true;
    }
...
::Encoding.register 'UTF-16BE', inherits: ::Encoding::UTF_16LE do
  ...
  def valid_encoding?(str)
    %x{
      let t_decoder = new TextDecoder('utf-16be', { fatal: true });
      try { t_decoder.decode(new Uint8Array(str.$bytes())); }
      catch { return false; }
      return true;
    }
...
::Encoding.register 'ASCII-8BIT', aliases: ['BINARY'], ascii: true do
  ...
  def valid_encoding?(str)
    %x{
      for (const c of str) { if (c.codePointAt(0) > 0xff) return false; }
      return true;
    }
  end
end
